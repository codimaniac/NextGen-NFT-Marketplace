        ...
        <>
                ...
                <div className="flex flex-col w-full gap-12 lg:flex-row">
                    ...
                    <div className="flex flex-col flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-8">
                            <img src={nft.creator_pfp} alt={nft.creator} className="rounded-full w-10"/>
                        ...
                        <div className="flex flex-col gap-1 bg-[#16192a] border-1 border-[#2e3150] p-8 mt-8 rounded-[10px] font-light text-sm leading-[183%] text-justify">
                            ...
                            <div className="flex items-center gap-2 my-4">
                                <img src={nft.owner_pfp} alt={nft.owner} className="rounded-full w-10"/>
                                <div className="flex flex-col">
                                    <span className="text-xs">@{nft.owner}</span>
                                    <span className="text-xs">Owner</span>
                                </div>
                            </div>
                            <div><span className="font-medium">Collection: </span>{nft.collections?.join(', ')}</div>
                            <div><span className="font-medium">Token ID: </span>{nft.id}</div>
                        </div>
            ...
            <div className="flex flex-col items-center gap-4 section__margin">                
                <ul className="flex flex-row justify-between w-full font-bold text-[var(--light-purple)] text-sm mt-2 md:justify-center md:gap-8 xl:gap-14">
                    ...
                    <li><Link to={`${basePath}/review`} className={`${path==`/nft/${id}/review` ? 'text-[var(--light-color)]' : ''} no-underline hover:text-[var(--accent-color)]`}>Review ({nft.review?.length})</Link></li>
                </ul>
                <Routes>
                    <Route index element={<ProductDescription full_desc={nft.full_desc}/>} />
                    <Route path="additional-info" element={<AdditionalInfo full_desc={nft.full_desc}/>} />
