...
const NFTsCarousel = ({ items, itemsPerSlide }) => {
  const [currentPage, currentSlide, slidePageNumbers, prevPage, nextPage] =
    useCarousel(items, itemsPerSlide);
  if (items.length == 0) {
    return (
      <div className="flex flex-col gap-4 w-full h-full items-center justify-center rounded-[10px] bg-[#16192a] border-2 border-[#2e3150] p-20 section__margin">
        <h3 className="text-[var(--lighter-color)] text-2xl text-center font-light leading-[183%]">
  ...
  return (
    <>
      <div className="flex flex-col items-center w-full gap-x-4 gap-y-40 sm:gap-y-32 mt-2 sm:flex-row sm:flex-wrap">
